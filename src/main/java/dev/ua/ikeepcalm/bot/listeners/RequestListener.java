...
@Component
public class RequestListener extends ListenerAdapter implements EventDispatcher {
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (componentId.split("-")[0].equals("mirequest")) {
            if (componentId.split("-")[1].equals("yes")) {
                ...
                String coordinates = componentId.split("-")[3];

                try (RconClient client = RconClient.open(rconUrl, rconPort, rconPassword)) {
                    client.sendCommand("tp " + nickname + " " + coordinates);
                } catch (Exception e) {
        ...
        } else if (componentId.split("-")[0].equals("arequest")) {
            if (componentId.split("-")[1].equals("yes")) {
                EmbedBuilder embedBuilder = new EmbedBuilder(event.getMessage().getEmbeds().getFirst());
                embedBuilder.setColor(Color.GREEN);
                embedBuilder.setFooter("Підтвердив: " + event.getUser().getName(), event.getUser().getAvatarUrl());
                ...
                String command = componentId.split("-")[2];
